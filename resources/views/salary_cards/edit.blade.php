    ...
    <div class="container-fluid">
        <div class="card card-primary card-outline">
            <div class="card-header">
                <h3 class="card-title">Edit Salary Card #{{ $salaryCard->id }}</h3>
                        ...
                        <form action="{{ route('salary-cards.update', $salaryCard->id) }}" method="POST">
                            ...
                            <div class="form-group mb-3">
                                ...
                                <select name="employee_id" id="employee_id" class="form-control" required>
                                    <option value="{{ $salaryCard->employee->id }}">
                                        {{ $salaryCard->employee->name }} ({{ $salaryCard->employee->email }})
                                    </option>
                                </select>
                                @error('employee_id')
                            ...
                            <div class="form-group mb-3">
                                ...
                                <input type="number" name="basic_salary" id="basic_salary" class="form-control"
                                    step="0.01" value="{{ old('basic_salary', $salaryCard->basic_salary) }}" required>
                                @error('basic_salary')
                                    <span class="text-danger">{{ $message }}</span>
                                @enderror
                            </div>
                                ...
                                <div class="form-group mb-3">
                                    ...
                                    <div class="input-group">
                                        <select name="components[{{ $component->id }}][type]"
                                            ...
                                            <option value="fixed" {{ $calculationType === 'fixed' ? 'selected' : '' }}>
                                                Fixed
                                                Amount
                                            </option>
                                            <option value="percentage"
                                                {{ $calculationType === 'percentage' ? 'selected' : '' }}>%
                                                of Basic
                                                Salary</option>
                                        </select>
